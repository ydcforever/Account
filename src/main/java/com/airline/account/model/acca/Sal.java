package com.airline.account.model.acca;

import lombok.Data;
import lombok.NoArgsConstructor;

/**
 * Created by ydc on 2020/9/15.
 */
@Data
@NoArgsConstructor
public class Sal {

    private String dataType;

    private String outputSys;

    private String balMonth;

    private String balPeriod;

    private String airline3code;

    private String ticketNo;

    private String financialNo;

    private String couponUseIndicator;

    private String firstTicketNo;

    private Integer cnjNo;

    private String saleType;

    private String ticketType;

    private String issueDate;

    private String agentNo;

    private String airport1;

    private String airport2;

    private String airport3;

    private String airport4;

    private String airport5;

    private String flightNo1;

    private String flightNo2;

    private String flightNo3;

    private String flightNo4;

    private String flightDt1;

    private String flightDt2;

    private String flightDt3;

    private String flightDt4;

    private String carrier1;

    private String carrier2;

    private String carrier3;

    private String carrier4;

    private String subClass1;

    private String subClass2;

    private String subClass3;

    private String subClass4;

    private String fareBasis1;

    private String fareBasis2;

    private String fareBasis3;

    private String fareBasis4;

    private String endorsementRestriction;

    private String fca;

    private String pnrNo;

    private String tourCode;

    private String keyAccount;

    private String etFlag;

    private String paxName;

    private String paxType;

    private String saleCurrency;

    private String incomePayType;

    private String paxQty;

    private String incomePay;

    private String incomePaySc;

    private String incomePax;

    private String incomePaxSc;

    private String tax;

    private String taxSc;

    private String agentCommissionRate;

    private String agentCommission;

    private String agentCommissionSc;

    private String addCommissionRate;

    private String addCommission;

    private String addCommissionSc;

    private String overweightLuggage;

    private String overLuggagePrice;

    private String overLuggageFee;

    private String valuableFeeFee;

    private String valuableFeeSc;

    private String exchOriginAirline;

    private String exchOriginTicketNo;

    private String exchOriginCnjNo;

    private String rfndAirline;

    private String rfndType;

    private String rfndDocumentNo;

    private String dataSource;

    private String srcAgentNo;

    private String creditCardNo;

    private String gpFlag;

    private String autoTicketFlag;

    private String batchNo;

    private String saleCity;

    private String sourceName;
}
